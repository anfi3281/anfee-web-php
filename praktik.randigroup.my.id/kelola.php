...
if (isset($_POST['ubah'])) {
    ...
    $hb = $_POST['hb'];
    $a = $_POST['a'];
    $query = "SELECT * FROM id_berjalan";
    $ambil = mysqli_query($host, $query);
    while($isi = mysqli_fetch_array($ambil)){
        $idb = $isi['0'];
        $query = "UPDATE tbobat set kd_barang='$kb', nm_barang='$nb', kd_satuan='$ks', kd_kategori='$kk', hrg_jual='$hj', hrg_beli='$hb', active='$a' WHERE kd_barang='$idb'";
